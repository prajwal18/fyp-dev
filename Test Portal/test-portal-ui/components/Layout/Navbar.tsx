...
//MUI icons
import { studentSidebarData, teacherSidebarData, isCurrentLocation } from "./NavigationUtils";            // Importing NavigationLink data from links
        ...
        <Stack sx={{ width: "100%", padding: "20px" }} spacing={1}>
            {
                sidebarData.map((item: any, index: number) => (
                    <React.Fragment key={index}>
                        <NavLinks href={item.path} selected={isCurrentLocation(asPath, item.path)}>
                            <item.icon/>
